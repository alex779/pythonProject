...
def cred_warn():
    ...
    for user in res_users['Users']:
        ...
        if (difference.days > min_idle_days) \
            ...
            user_list.append(user['UserName'])  # Collecting all accounts that meet the conditions
    ...
    BODY_TEXT = \
        (
            "Your Password for 472345033518 (Inveniam) AWS account is expiring in a few days and will be deactivated.\r\n"
            "Log into AWS and go to your IAM user to rotate your password:"
            " https://console.aws.amazon.com/iam/home?#security_credential")
    BODY_HTML = """
                            Your password need to be rotated in AWS Account: 472345033518 (Inveniam) AWS account as it is expiring soon. 
...
def cred_removal():
    ...
    for user in res_users['Users']:
        if 'PasswordLastUsed' in user:  # Checking for console user password last usage
            ...
            if (difference.days > max_idle_days) and (username.partition("@")[2] == "dev.pro"
                ...
                # This function is processing accounts with DevPro emails as a username only
                user_list.append(user['UserName'])  # Gathering all related accounts in a list to send notifications

                for login in user_list:
                    print('Attempting to remove login profile for {0}'.format(login))
                    try:
                        iam_client.delete_login_profile(  # Attempting to remove login profile for matching account
                            UserName=login
                        )
                        print('User {0} login profile has been removed'.format(login))
    ...
    SUBJECT = "IAM Password expiration"
    BODY_TEXT = \
        ("Your IAM credentials for 472345033518 (Inveniam) AWS account expired and have been deactivated.\r\n"
